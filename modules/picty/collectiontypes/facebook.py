...
##TODO: THIS IS FLICKR PERMISSION STUFF -- FACEBOOK OPERATES DIFFERENTLY
privacy_levels=[
'PUBLIC',
'FRIENDS AND FAMILY',
'FRIENDS',
...
##TODO: POOLS AND SETS ARE FLICK CONCEPTS
...
class FacebookOauthLoginDialog(gtk.Dialog):
    '''
    Dialog used to handle oauth (kind of clunky, would be better to show in the browser tab than as a popup)
    '''
    client_id = '1408244739405536' #picty's unique app id
    auth_url = 'https://www.facebook.com/dialog/oauth?response_type=token&display=page&scope=publish_actions,publish_stream,user_photos&client_id=%s&redirect_uri=https://www.facebook.com/connect/login_success.html'%(client_id,)
    def __init__(self):
        import webkit
        gtk.Dialog.__init__(self,title='Facebook Authentication',flags=gtk.DIALOG_NO_SEPARATOR|gtk.DIALOG_MODAL,
                         buttons=(gtk.STOCK_CANCEL, gtk.RESPONSE_REJECT))#,gtk.STOCK_REFRESH,gtk.RESPONSE))
        ...
        self.show_all()
        self.view.connect("document-load-finished",self.load_finished)
        self.view.open(self.auth_url)
        self.token = None

        #add reload and cancel buttons
    ...
    def load_finished(self,view,frame):
        ...
        urlp = urlparse(url)
        print urlp
        if urlp.netloc == 'www.facebook.com' and urlp.path == '/connect/login_success.html':
            fragments = urlp.fragment.split('&')
            for f in fragments:
                parm,value = f.split('=')
                if parm.strip() == 'access_token':
...
class LoadFacebookCollectionJob(backend.WorkerJob):
    def __init__(self,worker,collection,browser,rescan_at_open):
        backend.WorkerJob.__init__(self,'LOADFACEBOOKCOLLECTION',890,worker,collection,browser)
        self.pos=0
...
#        log.info('Loading collection '+self.collection_file)
        ...
        print 'OPENING FACEBOOK COLLECTION',collection.id,collection.type
        ...
        if collection._open():
            ...
            if self.collection.facebook_client!=None and (self.full_rescan or len(self.collection)==0):
                self.worker.queue_job_instance(FacebookSyncJob(self.worker,self.collection,self.browser))
...
class FacebookSyncJob(backend.WorkerJob):
    def __init__(self,worker,collection,browser):
        backend.WorkerJob.__init__(self,'FACEBOOKSYNC',700,worker,collection,browser)
        self.started=False

    def __call__(self):
        jobs=self.worker.jobs
        collection=self.collection
        facebook_client=collection.facebook_client
        recently_updated=False
        if not self.started:
            pluginmanager.mgr.suspend_collection_events(self.collection)
            self.page=0
            self.pages=1
            self.photodata=facebook_client.fql_query('SELECT object_id, aid, created, modified, caption, src_small, src_big FROM photo WHERE owner = me()')# OR owner IN (SELECT uid2 FROM friend WHERE uid1 = me())')
            self.counter=0
            self.started=True
        new_time=time.time()
        ...
        while jobs.ishighestpriority(self) and self.page<=self.pages:
            while self.counter<len(self.photodata) and jobs.ishighestpriority(self):
                ...
                gobject.idle_add(self.browser.update_status,pct,'Syncing with Facebook')
                ph=self.photodata[self.counter]
                self.counter+=1
                uid=str(ph.object_id)
                ...
                item.sync=True
                try:
                    datecreate=datetime.fromtimestamp(ph.created)
                except:
                    datecreate=None
                try:
                    datemod=datetime.fromtimestamp(ph.modified)
                except:
                    datemod=datecreate
                if item.meta==None or datemod!=item.meta['DateModified']:
                    collection.load_metadata(item,photo_data=ph)
                if ind<0:
                ...
                if self.counter == len(self.photodata):
                    pass
#                    ##TODO: Look for another page of data
#                    if len(self.photodata)>0:
#                        print 'NEXT PAGE'
#                        self.counter = 0
#                        self.pages+=1
#                        self.page+=1
...
#                gobject.idle_add(collection.sets.set_flickr_sets,sets) ##don't want to change liststore on a background thread
            ...
            return True
        gobject.idle_add(self.browser.update_status,2.0,'Pausing Facebook Syncing')
        gobject.idle_add(self.browser.update_backstatus,False,'Syncing Paused - %s'%(collection.name,))
        return False

##TODO: THIS IS FLICKR STUFF
class FacebookTransferOptionsBox(wb.VBox):
...
##TODO: THIS IS FLICKR STUFF
class FacebookMetadataWidget(wb.ModalDialog):
...
class FacebookPrefWidget(gtk.VBox):
...
#        self.name_entry.connect("changed",self.name_changed)
        self.rescan_check=gtk.CheckButton("Rescan facebook account for changes after opening")
...
class NewFacebookAccountWidget(gtk.VBox):
    def __init__(self,main_dialog,value_dict):
        gtk.VBox.__init__(self)
        self.main_dialog=main_dialog
        label=gtk.Label()
        label.set_markup("<b>Facebook Settings</b>")
...
def create_empty_collection(name,prefs,overwrite_if_exists=False):
    ...
    try:
        ...
        for p in FacebookCollection.pref_items + ('type',):
            if p in prefs:
                d[p]=prefs[p]
...
class FacebookCollection(baseobjects.CollectionBase):
    ...
    type='FACEBOOK'
    type_descr='Facebook Account'
    local_filesystem=False
    browser_sort_keys=facebook_sort_keys
    api_key = '1408244739405536'
    pref_widget=FacebookPrefWidget
    add_widget=NewFacebookAccountWidget
    metadata_widget=FacebookMetadataWidget
    transfer_widget=FacebookTransferOptionsBox
    persistent=True
    user_creatable=True
    ...
    def __init__(self,prefs): #todo: store base path for the collection
        ...
        self.login_username=''
        self.login_id=''
        self.facebook_client=None #will be none if not logged in
        ...
        self.image_dirs=[]
        self.sync_at_login=True #try to synchronize with Facebook after start up
        ...
        ## use facebook icon for the collection tab
        if self.pixbuf == None:
            self.pixbuf = 'facebook'
    ...
    def open(self,thread_manager,browser=None):
        ...
        j=LoadFacebookCollectionJob(thread_manager,self,browser,self.rescan_at_open)
        thread_manager.queue_job_instance(j)

    def _open(self):
        '''
        load the cached state of the facebook collection from a binary pickle file
        '''
        col_dir=os.path.join(settings.collections_dir,self.name)
        if self.is_open:
            return True
        try:
            ...
            print 'Loaded facebook collection version',version
            try:
    ...
    def close(self):
        ...
        '''
        print 'Closing Facebook Collection',self.name
        if not self.is_open:
            return True
        try:
            col_dir=os.path.join(settings.collections_dir,self.name)
    ...
    def rescan(self,thead_manager):
        if not self.online:
            return
        print 'starting facebook sync job'
        sj=FacebookSyncJob(thead_manager,self,self.browser)
        thead_manager.queue_job_instance(sj)

    ''' ********************************************************************
            METHODS TO SYNC THE COLLECTION WITH THE FACEBOOK ACCOUNT
    ******************************************************************** '''

    def login(self):
        '''
        initialize facebook client object and log into the facebook account
        '''
        print '#########FACEBOOK LOGIN#########'
        try:
            d = FacebookOauthLoginDialog()
            response=d.run()
            d.destroy()
            if response == gtk.RESPONSE_ACCEPT:
                ...
                return False
            self.facebook_client = pyfb.Pyfb(self.api_key, access_token=access_token)
            self.online=True
            return True
    ...
    def get_collections(self):
        try:
            '''
            get the collections from the facebook account
            collections is actually a tree so this doesn't make a lot of sense
            '''
            response=self.facebook_client.collections_getTree()
    ...
    def get_sets(self):
        try:
            response=self.facebook_client.photosets_getList()
    ...
    ''' ************************************************************************
                        ...
                        FOR FACEBOOK: POLL AT MOST ONCE PER HOUR
        ************************************************************************'''
    ...
    def copy_item(self,src_collection,src_item,prefs):
        ...
        #TODO: FIX ME (THIS IS FLICKR CODE)
        return False
        try:
            ...
            #do the upload to facebook
            print 'uploading',src_item.uid,'from file',filename,' privacy',public,family,friends,'tags',tags
            def progress_cb(progress,done):
                ##send notification
                pass
            photo_id=self.facebook_client.upload(filename=filename,title=title,description=description,tags=tags,
            ...
            if set_name:
                if sid==None:
                    result=self.facebook_client.photosets_create(title=set_name,primary_photo_id=photo_id)
                    ...
                    if self.sets.is_temp(sid):
                        ...
                        set_sets=True
            item.meta['Sets']=[sid]
            ##TODO: If set_tags is not True in the write_metadata call below, facebook will add any tags in the image metadata to the tags that were already set during upload
    ...
    def delete_item(self,item):
        ...
        ##TODO: THIS IS FLICKR CODE
        return false
        try:
            self.facebook_client.photos_delete(photo_id=item.uid)
    ...
    def make_thumbnail(self,item,interrupt_fn=None,force=False):
        ...
        if not force and item.thumburi:
            return True
        print 'Facebook Collection: creating thumb for',item
        try:
    ...
    def load_metadata(self,item,missing_only=False,notify_plugins=True,photo_data=None):
        'retrieve metadata for an item from the source'
        '''
        INFORMATION ABOUT A PHOTO IN FACEBOOKS FQL TABLES
        TABLE PHOTO
        object_id, unique photo id
        aid, album id
        created, timestamp
        modified, timestamp
        caption, title
        src_small, thumbnail url
        src_big, fullsize url
        TABLE PRIVACY
        TABLE PHOTO_TAG
        '''
        #result=self.facebook_client.photos_getInfo(photo_id=item.uid)
        ph=photo_data
        if ph is None:
            return False
        try:
        ...
        try:
            ...
            datemod=datecreate
        meta={}
        meta['DateTaken']=datecreate
        meta['DateModified']=datemod
        try:
            item.imageurl=ph.src_big
        except:
            item.imageurl=ph.src_small
        item.thumburl=ph.src_small
        if 'name' in dir(ph):
            meta['Title']=ph.caption
        meta['imtype']='image/jpeg'
...
#        result_c=self.facebook_client.photos_getAllContexts(photo_id=item.uid)
...
##        6,270
        ...
        if set_meta:
            if 'meta_backup' in item.__dict__:
                cp('Title')
                cp('ImageDescription')
            self.facebook_client.photos_setMeta(photo_id=item.uid,title=title,description=description)
        if set_tags:
            if 'meta_backup' in item.__dict__:
                cp('Keywords')
            self.facebook_client.photos_setTags(photo_id=item.uid,tags=tags)
        if set_perms:
            if 'meta_backup' in item.__dict__:
                cp('Privacy')
                cp('PermComment')
                cp('PermAddMeta')
            self.facebook_client.photos_setPerms(photo_id=item.uid,is_public=is_public,is_friend=is_friend,is_family=is_family,perm_comment=perm_comment,perm_addmeta=perm_addmeta)
        if set_rotate and rotate!=None:
            if 'meta_backup' in item.__dict__:
                cp('Orientation')
            self.facebook_client.photos_transform_rotate(photo_id=item.uid,degrees=rotate)
        if set_sets:
            ...
            for s in adders:
                if self.sets.is_temp(s):
                    ...
                    s=new_sid
                self.facebook_client.photosets_addPhoto(photoset_id=s,photo_id=item.uid)
            for s in removers:
                if not self.sets.is_temp(s):
                    self.facebook_client.photosets_removePhoto(photoset_id=s,photo_id=item.uid)
...
baseobjects.register_collection('FACEBOOK',FacebookCollection)
